 ...
 */
function getBasicAuthHeader(credentials) {
    const user = credentials.domain && !credentials.username.includes('\\')
 ...
 */
function ntlmGet(url, credentials) {
 ...
 */
async function basicGet(url, credentials) {
 ...
 */
function buildProjectsUrl(baseUrl, apiVersion, top) {
 ...
 */
function apiVersionsForSource(sourceId) {
    if (sourceId === 'tfs2017') {
 ...
 */
async function tfsGet(url, credentials) {
 ...
 */
async function fetchProjectsPayload(sourceId, credentials, top) {
 ...
 */
function summarizeTfsErrorBody(body) {
 ...
 */
export async function verifyCredentials(candidates) {
    if (!candidates.length) {
        throw new Error('Username and password are required');
    }
    const sourceIds = Object.keys(config.tfs.sources);
    const errors = [];
    for (const credentials of candidates) {
        for (const sourceId of sourceIds) {
            ...
            try {
                ...
                errors.push(`${credentials.displayName} @ ${source.label}: HTTP ${result.statusCode}`);
            }
            catch (error) {
    ...
    throw new Error(`Login failed against TFS. Prefer DOMAIN\\username (e.g. REALSOFT-ME\\loiy.shehadeh). Details: ${errors
        .slice(0, 6)
        .join('; ')}`);
}
/**
 ...
 */
export async function fetchProjectsFromSource(sourceId, credentials) {
    const source = config.tfs.sources[sourceId];
    const result = await fetchProjectsPayload(sourceId, credentials, 1000);
    if (result.statusCode < 200 || result.statusCode >= 300) {
        throw new Error(`Failed to load projects from ${source.label} (${result.statusCode}): ${summarizeTfsErrorBody(result.body)} [${result.url}]`);
    }
 ...
 */
export async function fetchAllProjects(credentials) {
    const sourceIds = Object.keys(config.tfs.sources);
    const results = await Promise.allSettled(sourceIds.map((id) => fetchProjectsFromSource(id, credentials)));
    const projects = [];
    const errors = [];
