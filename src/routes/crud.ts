 ...
 */
function mapSubProject(row: SubProjectRow) {
  return {
    id: row.id,
 ...
 */
function mapConnection(row: DbConnectionRow) {
  return {
    id: row.id,
 ...
 */
function isValidSource(source: string): source is TfsSourceId {
 ...
 */
router.get('/projects/:source/:projectId/subprojects', (req: Request, res: Response) => {
  ...
  const rows = getDb()
    ...
    .all(source, projectId) as SubProjectRow[];
 ...
 */
router.post('/projects/:source/:projectId/subprojects', (req: Request, res: Response) => {
  const source = paramValue(req.params.source);
  const projectId = paramValue(req.params.projectId);
  if (!isValidSource(source)) {
    res.status(400).json({ error: `Unknown TFS source: ${source}` });
  ...
  const result = getDb()
    .prepare(
      `INSERT INTO sub_projects (tfs_source, tfs_project_id, name, description)
       VALUES (?, ?, ?, ?)`
    )
  ...
  const row = getDb()
    ...
    .get(result.lastInsertRowid) as SubProjectRow;
 ...
 */
router.put('/subprojects/:id', (req: Request, res: Response) => {
  ...
  const existing = getDb()
    .prepare('SELECT * FROM sub_projects WHERE id = ?')
    .get(id) as SubProjectRow | undefined;
 ...
 */
router.delete('/subprojects/:id', (req: Request, res: Response) => {
  const id = Number(req.params.id);
  if (!Number.isInteger(id) || id < 1) {
    res.status(400).json({ error: 'Invalid sub-project id' });
    return;
 ...
 */
router.get('/subprojects/:id/connections', (req: Request, res: Response) => {
  const id = Number(req.params.id);
  if (!Number.isInteger(id) || id < 1) {
    res.status(400).json({ error: 'Invalid sub-project id' });
    return;
 ...
 */
router.post('/subprojects/:id/connections', (req: Request, res: Response) => {
  const id = Number(req.params.id);
  if (!Number.isInteger(id) || id < 1) {
    res.status(400).json({ error: 'Invalid sub-project id' });
    return;
  ...
  const row = getDb()
    ...
    .get(result.lastInsertRowid) as DbConnectionRow;
...
router.put('/connections/:id', (req: Request, res: Response) => {
  ...
  const password =
    req.body?.password !== undefined ? String(req.body.password) : existing.password;
  const provider =
 ...
 */
router.get('/subprojects/:id', (req: Request, res: Response) => {
  const id = Number(req.params.id);
  if (!Number.isInteger(id) || id < 1) {
    res.status(400).json({ error: 'Invalid sub-project id' });
    return;
